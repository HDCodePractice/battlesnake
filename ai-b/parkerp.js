function parkerp_check_snakes_on_path(snakes,path){
...
function parkerp_check_no_turn(gridSize,snake,apples,direction,snakes){
    ...
    for (var i = 0; i < apples.length; i++) {
        ...
        if (snake_head_row == apple_row ){
            ...
            if (parkerp_check_snakes_on_path(snakes,path)){
                return "";
            }
...
function parkerp_getDirection(gridSize,snake,apples,direction,snakes){
    if (direction == ""){
        direction = "r";
    }
    let d = parkerp_check_no_turn(gridSize,snake,apples,direction,snakes);
    print("d: "+d);
    if (d != ""){
        return d;
    }
...
function parkerp_newGame(){
    return;
}

ais['parkerp'] = {
    getDirection: parkerp_getDirection,
    newGame: parkerp_newGame
};